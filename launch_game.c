...
/*   Updated: 2025/08/11 20:42:27 by vahdekiv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	launch(t_game *game)
{
	...
	if (!(game->mlx_ptr = mlx_init(game->map->width * MAP_SQUARE,
...
static mlx_image_t	*load_textures(t_game *game, char *path)
{
	...
	if ((texture_ptr = mlx_load_png(path)) == NULL)
		return (NULL);
	if ((image_ptr = mlx_texture_to_image(game->mlx_ptr, texture_ptr)) == NULL)
	{
...
void	load_images(t_game *game)
{
	...
	game->closed_exit = load_textures(game, "./textures/closeddoor.png");
	...
	while (y < game->map->height)
	{
		...
		while (x < game->map->width)
		{
			...
			if (game->map->mapgrid[y][x] == 'E')
				mlx_image_to_window(game->mlx_ptr, game->closed_exit,
				x * MAP_SQUARE, y * MAP_SQUARE);
			x++;
		}
		y++;
